...
int CRF_Model::perform_BFGS() {
    ...
    return 0;
    */
    fprintf(stderr, "BFGS is not supported.\n");
    exit(1);
}
...
double CRF_Model::forward_prob(const int len) {
    for (int x = 0; x < len; x++) {
        ...
        for (int i = 0; i < _num_classes; i++) {
            ...
            if (x == 0) {
                ...
                //	for (vector<int>::const_iterator j = _tagrl[i].begin(); j !=
                //_tagrl[i].end(); j++){
                //	  sum += edge_weight[*j][i] * forward_cache[x-1][*j];
                //	}
            }
    ...
    double total = 0;
    for (int i = 0; i < _num_classes; i++) {
        total += forward_cache(len - 1, i) *
                 edge_weight(i, _num_classes + 1);  // EOS
    }
    ...
    for (int x = len - 1; x >= 0; x--) {
        for (int i = 0; i < _num_classes; i++) {
            ...
            if (x == len - 1) {
                ...
                //	for (vector<int>::const_iterator j = _taglr[i].begin(); j !=
                //_taglr[i].end(); j++){
                //	  sum += edge_weight[i][*j] * backward_cache[x+1][*j];
                //	}
            }
...
double CRF_Model::forward_backward(const Sequence& seq) {
    ...
    */
    ...
    if (!(fp > 0 && fp < DBL_MAX && bp > 0 && bp < DBL_MAX)) {
        cerr << endl << "error: line:" << _line_counter
             << " floating overflow. a different value of Gaussian prior might "
                "work." << endl;
        ////cerr << endl << "error: floating overflow.  " << endl;
        return 1.f;  // may cause assert failure because
                     // forward_cache/backward_cache are also overflow
    }
...
double CRF_Model::viterbi(const Sequence& seq, vector<int>& best_seq) {
    ...
    double m = -DBL_MAX;
    for (int i = 0; i < _num_classes; i++) {
        double score = forward_cache(len - 1, i) *
                       edge_weight(i, _num_classes + 1);  // EOS
        if (score > m) {
            m = score;
            best_seq[len - 1] = i;
...
void CRF_Model::nbest_search(const double lb, const int len, const int x,
                             ...
                             vector<Path>& vp) {
    if (x < len && forward_cache(x, y) * rhs_score < lb)
        return;
...
double CRF_Model::nbest(const Sequence& seq, vector<Path>& vp,
    ...
    for (double lb = 0.3 * m;; lb *= 0.3) {
        ...
        //    double sum = 0;
        //    for (vector<Path>::const_iterator i = vp.begin(); i != vp.end();
        //    i++)
        //      sum += i->score;
        //    if (abs(sum - 1.0) < 0.000000001) break;
        if (iter++ > 1000)
            break;
        if ((int)vp.size() >= max_num)
            break;
        if (lb <= min_prob)
            break;
    }
...
int CRF_Model::make_feature_bag(const int cutoff) {
    ...
    if (cutoff > 0) {
        for (vector<Sequence>::const_iterator k = _vs.begin(); k != _vs.end();
             k++) {
            for (vector<Sample>::const_iterator i = k->vs.begin();
                 i != k->vs.end(); i++) {
                for (vector<int>::const_iterator j =
                         i->positive_features.begin();
...
// vector<double> CRF_Model::calc_state_weight(const Sequence & seq, const int
// i) const
vector<double> CRF_Model::calc_state_weight(const int i) const {
    vector<double> wsum(_num_classes);
    for (int j = 0; j < _num_classes; j++) {
        wsum[j] =
            forward_cache(i, j) / state_weight(i, j) * backward_cache(i, j);
    }
...
void CRF_Model::add_sample_empirical_expectation(const Sequence& seq,
    ...
    for (size_t i = 0; i < seq.vs.size(); i++) {
        for (vector<int>::const_iterator j =
                 seq.vs[i].positive_features.begin();
    ...
    if (USE_BOS_EOS) {
        ...
        _vee[edge_feature_id(seq.vs[seq.vs.size() - 1].label,
                             _num_classes + 1)] += 1.0;
    }
}
...
void CRF_Model::add_training_sample(const CRF_Sequence& seq) {
    ...
    Sequence s1;
    for (vector<CRF_State>::const_iterator i = seq.vs.begin();
         i != seq.vs.end(); i++) {
        ...
        if (contain_space(i->label)) {
            cerr << "error: the name of a label must not contain any space."
                 << endl;
            exit(1);
        }
        Sample s;
        ...
        for (vector<string>::const_iterator j = i->features.begin();
             j != i->features.end(); j++) {
            if (contain_space(*j)) {
                cerr << "error: the name of a feature must not contain any "
                        "space." << endl;
                exit(1);
            }
...
void CRF_Model::get_features(list<pair<pair<string, string>, double>>& fl) {
    fl.clear();
    for (StringStore::const_iterator i = _featurename_bag.begin();
...
bool CRF_Model::save_to_file(const string& filename, const double th) const {
    ...
    //  for (MiniStringBag::map_type::const_iterator i =
    //  _featurename_bag.begin();
    for (StringStore::const_iterator i = _featurename_bag.begin();
         i != _featurename_bag.end(); i++) {
        for (int j = 0; j < _label_bag.Size(); j++) {
            ...
            if (id < 0)
                continue;
            if (_vl[id] == 0)
                continue;  // ignore zero-weight features
            if (abs(_vl[id]) < th)
                continue;  // cut off low-weight features
            fprintf(fp, "%s\t%s\t%f\n", label.c_str(), history.c_str(),
                    _vl[id]);
        }
    }
...
void CRF_Model::decode_forward_backward(CRF_Sequence& s0,
    ...
    for (size_t i = 0; i < seq.vs.size(); i++) {
        ...
        if (OUTPUT_MARGINAL_PROB) {
            double sum = 0;
            for (vector<double>::const_iterator j = wsum.begin();
                 j != wsum.end(); j++)
                sum += *j;
            s0.vs[i].label = "";
        ...
        for (vector<Sequence>::const_iterator n = _vs.begin(); n != _vs.end();
            ...
            for (size_t i = 0; i < seq.vs.size(); i++) {
                ...
                if (s.label == vs[i])
                    continue;
                for (vector<int>::const_iterator j =
                         s.positive_features.begin();
                     j != s.positive_features.end(); j++) {
                    for (vector<int>::const_iterator k =
                             _feature2mef[*j].begin();
                         k != _feature2mef[*j].end(); k++) {
            ...
            for (int i = 0; i < seq.vs.size() - 1; i++) {
                const int eid0 =
                    edge_feature_id(seq.vs[i].label, seq.vs[i + 1].label);
            ...
            double wX = 0, X2 = 0;
            for (map<int, double>::const_iterator i = X.begin(); i != X.end();
                 i++) {
