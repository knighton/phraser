...
static void read_tagged(istream* ifile, vector<Sentence>& vs) {
...
static void print_help() {
    ...
    cout << "The format for each line is: WORD1/POS1 WORD2/POS2 WORD3/POS3 ..."
         << endl;
    cout << endl;
    cout << "Mandatory arguments to long options are mandatory for short "
            "options too." << endl;
    cout
        << "  -m, --model=DIR       specify the directory for saving the models"
        << endl;
    cout << "  -c, --compact         build compact models" << endl;
    cout << "  -f, --fast            build only the model for the fast mode"
         << endl;
...
int main(int argc, char** argv) {
    ...
    if (trains.size() == 0) {
        if (ifilename != "") {
            cerr << "error: cannot read \"" << ifilename << "\"" << endl;
            exit(1);
        }
    ...
    cerr << endl;
    cerr << "the models have been saved in the directory \"" << MODEL_DIR
         << "\"." << endl;
}
