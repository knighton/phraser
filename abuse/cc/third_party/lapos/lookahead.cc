...
double CRF_Model::lookahead_search(const Sequence& seq, vector<int>& history,
    ...
    if (forbidden_seq && depth == 1) {
        if ((*forbidden_seq)[depth - 1] !=
            history[HV_OFFSET + start + depth - 1])
            forbidden_seq = NULL;
    }
    ...
    for (int i = 0; i < _num_classes; i++) {
        if (follow_gold && i != seq.vs[start + depth].label)
            continue;
        ...
        if (depth + start > 0)
            new_score += _vl[edge_feature_id2(
        ...
        if (USE_EDGE_TRIGRAMS) {
            if (depth + start > 1)
                new_score += _vl[edge_feature_id3(
                    history[HV_OFFSET + start + depth - 3],
                    history[HV_OFFSET + start + depth - 2],
                    history[HV_OFFSET + start + depth - 1], i)];
        }
        ...
        vector<int> tmp_seq;
        const double score =
            lookahead_search(seq, history, start, max_depth, depth + 1,
                             new_score, tmp_seq, false, forbidden_seq);
        //    const double score = lookahead_search(seq, history, start,
        //    max_depth, depth + 1, new_score, tmp_seq, follow_gold,
        //    forbidden_seq);
        if (score > m) {
            m = score;
            best_seq.clear();
...
void CRF_Model::calc_diff(const double val, const Sequence& seq,
                          ...
                          map<int, double>& diff) {
    if (start + depth == (int)seq.vs.size())
        return;
...
static void print_bestsq(const vector<int>& bestsq) {
    for (vector<int>::const_iterator i = bestsq.begin(); i != bestsq.end();
         i++) {
        cout << *i << " ";
    }
    cout << endl;
...
int CRF_Model::update_weights_sub2(const Sequence& seq, vector<int>& history,
    ...
    vector<int> gold_seq;
    const double gold_score = lookahead_search(seq, history, x, LOOKAHEAD_DEPTH,
                                               0, 0, gold_seq, true);
    ...
    vector<int> best_seq;
    const double score = lookahead_search(seq, history, x, LOOKAHEAD_DEPTH, 0,
                                          0, best_seq, false, &gold_seq);
...
int CRF_Model::perform_LookaheadTraining() {
    ...
    while (iter < PERCEPTRON_NITER) {
        ...
        for (int i = 0; i < (int)_vs.size(); i++) {
            ...
            /*
            for (map<int, double>::const_iterator i = diff.begin(); i !=
          diff.end(); i++) {
          //	    cout << "(" << i->first << ", " << i->second << ") ";
          const double v = 1.0 * i->second;
          _vl[i->first] += v;
