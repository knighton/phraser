...
static string MODEL_DIR = ".";  // the default directory for saving the models
...
[[noreturn]] static void print_help() {
    ...
    cout << "By default, the tagger assumes that FILE contains one sentence "
            "per line" << endl;
    cout << "and the words are tokenized with white spaces." << endl;
    //  cout << "Try -t and -s options if you want the tagger to process raw
    //  text." << endl;
    cout << "Use -t option if you want to process untokenized sentences."
         << endl;
    cout << endl;
    cout << "Mandatory arguments to long options are mandatory for short "
            "options too." << endl;
    cout << "  -m, --model=DIR        specify the directory containing the "
            "models" << endl;
...
struct TagProb {
    ...
    bool operator<(const TagProb& x) const {
        return prob > x.prob;
    }
};
...
int main(int argc, char** argv) {
    ...
    while (getline(*is, line)) {
        ...
        if (false) {
            ...
            for (vector<Token>::const_iterator i = vt.begin(); i != vt.end();
                 i++) {
        ...
        for (size_t i = 0; i < vt.size(); i++) {
            ...
            double sum = 0;
            for (map<string, double>::const_iterator j = crf.begin();
                 j != crf.end(); j++) {
            ...
            for (map<string, double>::const_iterator j = ef.begin();
                 j != ef.end(); j++) {
                //	cout << j->first << ":" << j->second << " ";
                sum += j->second;
            ...
            for (map<string, double>::iterator j = m.begin(); j != m.end();
                 j++) {
                if (j->second >= th)
                    m2.insert(*j);
            }
            double maxp = -1;
            string maxtag;
            for (map<string, double>::iterator j = m2.begin(); j != m2.end();
                 j++) {
        ...
        for (size_t i = 0; i < vt.size(); i++) {
            ...
            if (STANDOFF || OUTPUT_TAG_PROBS || UIMA || ENJU) {
                ...
                if (OUTPUT_TAG_PROBS) {
                    vector<TagProb> tp;
                    double sum = 0;
                    for (map<string, double>::iterator j = tagp[i].begin();
                         j != tagp[i].end(); j++) {
                        tp.push_back(TagProb(j->first, j->second));
                        sum += j->second;
                    }
                    sort(tp.begin(), tp.end());
                    for (vector<TagProb>::iterator j = tp.begin();
                         j != tp.end(); j++) {
                        const double norm_p = j->prob / sum;  // normalize
                        if (norm_p == 1)
