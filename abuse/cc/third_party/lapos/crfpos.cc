...
static CRF_State crfstate(const vector<Token>& vt, int i) {
    ...
    for (size_t j = 1; j <= 10; j++) {
        ...
        //    if (str.size() > j+1) {
        if (str.size() >= j) {
            sprintf(buf, "SUF%d_%s", (int)j,
                    str.substr(str.size() - j).c_str());
    ...
    if (WNdic.size() > 0) {
        ...
        for (auto it = WNdic.lower_bound(n); it != WNdic.upper_bound(n); ++it) {
            sample.add_feature("WN_" + it->second);
        }
    }
    ...
    //  cout << sample.label << "\t";
    //  for (vector<string>::const_iterator j = sample.features.begin(); j !=
    //  sample.features.end(); j++) {
    //      cout << *j << " ";
    //  }
    //  cout << endl;
...
int crftrain(const CRF_Model::OptimizationMethod method, CRF_Model& m,
    ...
    if (method != CRF_Model::BFGS && use_l1) {
        cerr << "error: L1 regularization is currently not supported in this "
                "mode. Please use other optimziation methods." << endl;
        exit(1);
    }
