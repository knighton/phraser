...
static void evaluate(const vector<Sentence>& vs) {
    ...
    for (vector<Sentence>::const_iterator i = vs.begin(); i != vs.end(); i++) {
        ...
        for (unsigned int j = 0; j < s.size(); j++) {
            ...
            bool isknown = false;
            if (known_words.find(str) != known_words.end())
                isknown = true;
            if (isknown)
                ntotal_known++;
            else
            ...
            //      cout << s[j].str << "\t" << s[j].pos << "\t" << s[j].prd <<
            //      endl;
        }
        if (all_correct)
            ncorrect_sen++;
    }
    fprintf(stderr, "sentence  %7d / %7d = %f\n", ncorrect_sen, (int)vs.size(),
            (double)ncorrect_sen / vs.size());
    fprintf(stderr, "token     %7d / %7d = %f\n", ncorrect, ntotal,
            (double)ncorrect / ntotal);
    if (known_words.size() > 0) {
        fprintf(stderr, "  known   %7d / %7d = %f\n", ncorrect_known,
                ntotal_known, (double)ncorrect_known / ntotal_known);
        fprintf(stderr, "  unknown %7d / %7d = %f\n", ncorrect_unknown,
                ntotal_unknown, (double)ncorrect_unknown / ntotal_unknown);
    }
}

static void read_pos(const string& filename, vector<Sentence>& vs) {
...
int main(int argc, char** argv) {
    ...
    if (argc == 4) {
        ...
        vector<Sentence> tr;
        read_pos(trfile, tr);
        for (vector<Sentence>::const_iterator i = tr.begin(); i != tr.end();
             i++) {
    ...
    for (unsigned int i = 0; i < gs.size(); i++) {
        ...
        if (s0.size() != s1.size()) {
            cerr << "error: sentences do not match at line " << i + 1 << endl;
            exit(1);
        }
