...
class TokenizerData(object):
    def __init__(
        ...
        self.brackets_s2s = brackets_s2s
        self.americanize_s2s = americanize_s2s
    ...
    def generate_cpp(self):
        ...
        cc_lines.append('#include "tokenizer_data.h"')
        cc_lines.append('')
        ...
        token2token = dict(self.brackets_s2s)
        for from_s, to_s in self.americanize_s2s.iteritems():
            assert from_s not in token2token
            token2token[from_s] = to_s
...
def main():
    ...
    # ASCII -> ASCII (postprocess)
    brackets_s2s = load_brackets('brackets.txt')
    ...
    open('../tokenizer_data.h', 'wb').write(h)
    open('../tokenizer_data.cc', 'wb').write(cc)
